 ...
 */
function tkmulti_amp_class( $default, $active, $state ) {
	$output = '';
	if( tkmulti_is_amp() ) {
		$output .= sprintf(
			' [class]="%s"',
 ...
 */
function tkmulti_amp_toggle( $state = '', $disable = array() ) {
	if( ! tkmulti_is_amp() )
		return;
 ...
 */
function tkmulti_amp_nav_dropdown( $theme_location = false, $depth = 0 ) {
	...
	return tkmulti_amp_toggle( $key ) . tkmulti_amp_class( 'submenu-expand', 'expanded', $key );
}
