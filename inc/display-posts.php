 ...
 */
function tkmulti_dps_template_part( $output, $original_atts ) {
...
add_action( 'display_posts_shortcode_output', 'tkmulti_dps_template_part', 10, 2 );

/**
 * Display Posts Wrapper
 *
 */
function tkmulti_dps_wrapper_open( $markup, $atts ) {
...
add_filter( 'display_posts_shortcode_wrapper_open', 'tkmulti_dps_wrapper_open', 10, 2 );

/**
 * Display Posts, wrapper close
 *
 */
function tkmulti_dps_wrapper_close( $markup, $atts ) {
	if( empty( $atts['layout'] ) )
		return $markup;

	return '</div>';
}
add_filter( 'display_posts_shortcode_wrapper_close', 'tkmulti_dps_wrapper_close', 10, 2 );
